...
<script>
    ...
    export default {
        ...
        methods: {
            match_video_query(videos, querys) { // takes in videos and objs, return videos with satisfied objs
            ...
            do_sat(obj, key = null, query_key = null, condition = {}) {
                ...
                if (key == null) {
                    ...
                    if (typeof condition == "number" || typeof condition == "string" || typeof condition == "boolean" || (typeof condition == "object" && "$date" in condition)) { // tags: 1
                        if (typeof this.get_obj(obj, key) == "object" && Array.isArray(this.get_obj(obj, key))) {// array
                            ...
                            if (query_key == null || query_key == "$eq") { // key: value or {$eq: val}
                                return lhs == rhs;
                            } else if (query_key == "$gt") { // {$gt: val}
                                return lhs > rhs;
                            } else if (query_key == "$lt") { // {$lt: val}
                                return lhs < rhs;
                            } else if (query_key == "$gte") { // {$gte: val}
                                return lhs >= rhs;
                            } else if (query_key == "$lte") { // {$lte: val}
                                return lhs <= rhs;
                            }
                        }
                    } else if (Array.isArray(condition)) { // tags: {$all: [1, 2]} or tags: {$in: [1, 2]} or tags: {$nin: [1, 2]}
                        if (query_key == "$all") {
